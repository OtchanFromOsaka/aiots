...
describe("postForm", () => {
	...
	it("should send expected POST request", async () => {
		const response = await postForm("http://example.com");
	...
	it("should send expected POST request with query", async () => {
		...
		const response = await postForm("http://example.com", option);
		expect(response.status).toEqual(200);
		const url = mockedFetch.mock.calls[0][0];
	...
	it("should send expected POST request with form body", async () => {
		...
		expect(body.toString()).toEqual("id=123&name=user%40test.com");
	});
});
